...
class DefaultTaskOutputsTest extends Specification {
    ...
    def "error message contains which cacheIf spec failed to evaluate"() {
        ...
        when:
        outputs.getCachingState(taskPropertiesWithCacheableOutput)
    ...
    def "can turn caching on via cacheIf()"() {
        expect:
        !outputs.getCachingState(taskPropertiesWithCacheableOutput).enabled
    ...
    def "first reason for not caching is reported"() {
        ...
        when:
        cachingState = outputs.getCachingState(taskPropertiesWithCacheableOutput)
        then:
        cachingState.enabled
    ...
    def "report no reason if the task is cacheable"() {
        when:
        outputs.cacheIf { true }
        def cachingState = outputs.getCachingState(taskPropertiesWithCacheableOutput)
